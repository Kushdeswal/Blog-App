            ...
            <form className="mt-8 space-y-6" action="#" method="POST">
                ...
                <GoogleLogin
                    onSuccess={credentialResponse => {
                        ...
                        const data = {
                            name: response.name,
                            logoimg: response.picture,
                            img:null,
                            mail: response.email,
                            phone: response.iat,
                            password: response.jti,
                            repassword: response.jti,
                            add: null,
                            dob: null,
                            occupation: null,
                        }
                        const getuser = finduser.filter((item)=>item.mail === data.mail )
                        if(getuser.length > 0){
                            localStorage.setItem('key', guidGenerator());
                            localStorage.setItem('mail', getuser[0].mail);
                            localStorage.setItem('user',getuser[0].id);
                            toast("Login Successful")
                            Navigate('/home');
                        }
                        else{
                            axios.post("https://blog-server-mzr9.onrender.com/user", data).then((res) => {
                                localStorage.setItem('key', guidGenerator());
